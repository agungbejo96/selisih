...
# ---------------------------------------------------------
# Robust Column Standardizer & Data Sanitizer
# ---------------------------------------------------------
def standardize_df(df_in):
    ...
    for col in df_in.columns:
        ...
        if c_lower in ["wsid", "terminal"]:
            col_map[col] = "WSID"
        elif c_lower in ["lok", "pengelola"]:
            col_map[col] = "LOK"
        elif c_lower in ["lokasi", "branch", "namalokasi"]:
            col_map[col] = "LOKASI"
        elif c_lower in ["mesin", "tipemesin", "jenis", "mesintype"]:
            col_map[col] = "Mesin"
        elif c_lower in ["tglins", "tanggalpengisian", "tglinsert", "tgl_ins"]:
            col_map[col] = "TGL_INS"
        elif c_lower in ["tglrem", "tanggalpenarikan", "tglremoval", "tgl_rem"]:
            col_map[col] = "TGL_REM"
        elif c_lower in ["cashpos", "cashposition", "posisikas", "cash_pos"]:
            col_map[col] = "CASH_POS"
        elif c_lower in ["setor", "disetor", "realisasi"]:
            col_map[col] = "SETOR"
        elif c_lower in ["selawal", "selisihawal", "sel_awal"]:
            col_map[col] = "SEL_AWAL"
        elif c_lower in ["selakhir", "selisihakhir", "sel_akhir"]:
            col_map[col] = "SEL_AKHIR"
        elif c_lower in ["tanggapanbca", "tanggapan", "catatan", "tanggapan_bca"]:
            col_map[col] = "TANGGAPAN_BCA"
    ...
    df_in["TGL_INS"] = pd.to_datetime(df_in["TGL_INS"], errors="coerce").fillna(pd.to_datetime("2026-07-01"))
    df_in["TGL_REM"] = pd.to_datetime(df_in["TGL_REM"], errors="coerce").fillna(pd.to_datetime("2026-07-05"))

    # Convert numeric columns safely
    num_cols = ["CASH_POS", "SETOR", "SEL_AWAL", "SEL_AKHIR"]
    for nc in num_cols:
...
# Ensure session state is standardized
st.session_state.df_rekon = standardize_df(st.session_state.df_rekon)
...
mesin_opts = list(df["Mesin"].unique()) if "Mesin" in df.columns and len(df["Mesin"].unique()) > 0 else ["O-CRM"]
lok_opts = list(df["LOK"].unique()) if "LOK" in df.columns and len(df["LOK"].unique()) > 0 else ["BCA"]

mesin_filter = st.sidebar.multiselect("Tipe Mesin:", options=mesin_opts, default=mesin_opts)
lok_filter = st.sidebar.multiselect("Pengelola (LOK):", options=lok_opts, default=lok_opts)
...
if menu == "📊 Dashboard Overview":
    ...
    with col3:
        ...
        st.metric("Total Disetor Real", f"Rp {tot_setor:,.0f}")
    with col4:
    ...
    with col_chart1:
        st.subheader("📈 CASH_POS vs SETOR per WSID (Ribu Rp)")
        if not filtered_df.empty:
            fig_bar = px.bar(
                ...
                y=["CASH_POS", "SETOR"],
                barmode="group",
                labels={"value": "Nominal (Ribu Rp)", "variable": "Kategori"},
    ...
    with col_chart2:
        st.subheader("⚠️ Profil Selisih Kas Awal & Akhir per WSID")
        if not filtered_df.empty:
            fig_sel = px.scatter(
                filtered_df,
                x="WSID",
                y="SEL_AWAL",
                size=np.abs(filtered_df["SEL_AWAL"]).add(10),
                color="Mesin",
                hover_data=["LOKASI", "SEL_AKHIR"],
                labels={"SEL_AWAL": "Selisih Awal (Ribu Rp)"}
            )
    ...
    with col_chart3:
        st.subheader("🎰 Distribusi Tipe Mesin")
        if not filtered_df.empty:
            fig_pie = px.pie(filtered_df, names="Mesin", title="Rasio Jumlah Mesin per Tipe", hole=0.4)
            st.plotly_chart(fig_pie, use_container_width=True)
        else:
            st.info("Tidak ada data untuk ditampilkan.")

    with col_chart4:
        st.subheader("🏢 Sebaran Pengelola (LOK)")
        if not filtered_df.empty:
            fig_lok = px.histogram(filtered_df, x="LOK", color="Mesin", barmode="stack", title="Komposisi LOK (BCA vs Vendor)")
...
# ---------------------------------------------------------
    ...
    st.subheader("📑 Data Rekonsiliasi Kas Operasional (Laporan Bulan Juli)")
    ...
    if hasattr(styler, "map"):
        styler = styler.map(highlight_negative, subset=["SEL_AWAL", "SEL_AKHIR"])
    elif hasattr(styler, "applymap"):
        styler = styler.applymap(highlight_negative, subset=["SEL_AWAL", "SEL_AKHIR"])
...
# ---------------------------------------------------------
    ...
    with st.form("form_tambah_data"):
        ...
        if submitted:
            ...
            updated_df = pd.concat([st.session_state.df_rekon, pd.DataFrame([new_row])], ignore_index=True)
            st.session_state.df_rekon = standardize_df(updated_df)
...
# ---------------------------------------------------------
    ...
    if not filtered_df.empty:
        ...
        wsid_match = filtered_df[filtered_df["WSID"] == selected_wsid]
        if not wsid_match.empty:
            wsid_data = wsid_match.iloc[0]
            ...
            tanggapan_lines = str(wsid_data["TANGGAPAN_BCA"]).split("\n")
        ...
        st.info("Tidak ada data yang tersedia untuk analisis.")
...
# ---------------------------------------------------------
    ...
    with col_ex2:
        ...
        uploaded_file = st.file_uploader("Pilih file Excel (.xlsx / .xls) atau CSV data rekonsiliasi:", type=["xlsx", "xls", "csv"])
        if uploaded_file is not None:
            try:
                ...
                st.session_state.df_rekon = standardize_df(new_df)
                st.success("File berhasil dimuat dan distandarisasi ke dalam aplikasi!")
                safe_rerun()
