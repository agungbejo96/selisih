...
# ---------------------------------------------------------
st.set_page_config(
    page_title="Sistem Rekonsiliasi Kas BCA + Input Data Terpadu",
    page_icon="🏦",
    layout="wide",
...
menu = st.sidebar.radio(
    ...
    [
        ...
        "📸 Galeri Foto Bukti Fisik",
        "➕ Menu Input Data Terpadu",
...
# ---------------------------------------------------------
# MODUL 2: TABEL UTAMA & STAFF OPERASIONAL
# ---------------------------------------------------------
    ...
    if selected_wsid in st.session_state.image_store:
        ...
        st.info(f"💡 Belum ada foto bukti khusus yang diunggah untuk WSID {selected_wsid}. Anda dapat mengunggahnya pada menu **➕ Menu Input Data Terpadu**.")
...
# ---------------------------------------------------------
# MODUL 9: GALERI FOTO BUKTI FISIK
# ---------------------------------------------------------
elif menu == "📸 Galeri Foto Bukti Fisik":
    st.subheader("📸 Galeri & Preview Lampiran Foto Bukti Fisik")
    st.write("Fitur untuk melihat foto bukti fisik (papan catatan kas, uang kolong, struk) per WSID.")

    view_wsid = st.selectbox("Pilih WSID untuk Pratinjau Foto Bukti:", df_rekon["WSID"].unique(), key="preview_wsid_galeri")
    ...
    if view_wsid in st.session_state.image_store:
        ...
        st.info(f"Belum ada foto bukti yang diunggah untuk WSID {view_wsid}. Anda bisa mengunggahnya pada menu **➕ Menu Input Data Terpadu**.")

# ---------------------------------------------------------
# MODUL 10: MENU INPUT DATA TERPADU (FULL INTEGRATED)
# ---------------------------------------------------------
elif menu == "➕ Menu Input Data Terpadu":
    st.subheader("➕ Pusat Penginputan Data Operasional Rekonsiliasi & Upload Gambar")
    st.write("Silakan pilih tab di bawah ini untuk memasukkan data transaksi baru atau mengunggah bukti foto untuk setiap modul tabel.")
    ...
    # ---------------- TAB 1: REKON UTAMA & STAFF ----------------
    with tab1:
        st.markdown("### 📋 Form Input Rekonsiliasi Utama & Staff Operasional")
        with st.form("form_input_rekon_staff"):
            ...
            uploaded_file_tab1 = st.file_uploader("📸 Upload Lampiran Foto Bukti (Opsional):", type=["png", "jpg", "jpeg"], key="upload_tab1")

            submitted_rekon = st.form_submit_button("💾 Simpan Transaksi Rekonsiliasi & Staff")
            ...
            if submitted_rekon:
                ...
                if uploaded_file_tab1 is not None:
                    st.session_state.image_store[new_wsid] = uploaded_file_tab1.read()
                    
                st.success(f"Data Rekonsiliasi WSID {new_wsid} berhasil disimpan!")
                safe_rerun()

    # ---------------- TAB 2: CLOSE/OPEN CENCON ----------------
    with tab2:
        st.markdown("### 💵 Form Input Close / Open Cencon & Tes Cash")
        with st.form("form_input_cencon_tab"):
            c_c1, c_c2, c_c3, c_c4 = st.columns(4)
            with c_c1:
                c_wsid = st.text_input("WSID Terminal", "Z5UM", key="c_wsid_tab")
                c_lokasi = st.text_input("Nama Lokasi", "CRM PURWOSARI 2", key="c_lokasi_tab")
            with c_c2:
                c_tgl = st.date_input("Tanggal Transaksi", key="c_tgl_tab")
                c_jam = st.text_input("Jam (HH:MM)", "20:22", key="c_jam_tab")
            with c_c3:
                c_bdc = st.number_input("Nominal BDC (Rp)", value=0, step=10000, key="c_bdc_tab")
                c_non_bdc = st.number_input("Nominal NON-BDC (Rp)", value=100000, step=10000, key="c_non_bdc_tab")
            with c_c4:
                c_status = st.selectbox("Status Cencon", ["Close", "Open", "Pending"], key="c_status_tab")
                c_ket = st.text_input("Keterangan Tes Cash", "Tes cash sesuai 100,000", key="c_ket_tab")

            uploaded_file_tab2 = st.file_uploader("📸 Upload Foto Bukti Struk/Tes Cash (Opsional):", type=["png", "jpg", "jpeg"], key="upload_tab2")

            c_submit = st.form_submit_button("💾 Simpan Record Cencon")
            if c_submit:
                new_c = {
                ...
                if uploaded_file_tab2 is not None:
                    st.session_state.image_store[c_wsid] = uploaded_file_tab2.read()
                    
                st.success("Data Cencon berhasil ditambahkan!")
                safe_rerun()

    # ---------------- TAB 3: UANG KEMBALI (UK) ----------------
    with tab3:
        st.markdown("### 🔄 Form Input Klaim Uang Kembali (UK TC, UK BDC & UK Siclus)")
        with st.form("form_input_uk_tab"):
            u_1, u_2, u_3, u_4 = st.columns(4)
            with u_1:
                uk_wsid = st.text_input("WSID Terminal", "ZM16", key="uk_wsid_tab")
                uk_lokasi = st.text_input("Nama Lokasi", "ALFAMD INDRAGIRI PASURUAN", key="uk_lokasi_tab")
            with u_2:
                uk_tipe = st.selectbox("Tipe UK", ["UK Siclus", "UK TC", "UK BDC"], key="uk_tipe_tab")
                uk_tgl = st.date_input("Tanggal Klaim", key="uk_tgl_tab")
            with u_3:
                uk_jam = st.text_input("Jam Transaksi", "13:20", key="uk_jam_tab")
                uk_bdc = st.number_input("Nominal UK BDC (Rp)", value=0, step=50000, key="uk_bdc_tab")
            with u_4:
                uk_non_bdc = st.number_input("Nominal UK Non-BDC (Rp)", value=100000, step=50000, key="uk_non_bdc_tab")
                uk_ket = st.text_input("Keterangan Rincian UK", "UK d.50 x 2 lbr", key="uk_ket_tab")

            uploaded_file_tab3 = st.file_uploader("📸 Upload Foto Bukti Lampiran UK (Opsional):", type=["png", "jpg", "jpeg"], key="upload_tab3")

            uk_submit = st.form_submit_button("💾 Simpan Record Uang Kembali (UK)")
            if uk_submit:
                ...
                if uploaded_file_tab3 is not None:
                    st.session_state.image_store[uk_wsid] = uploaded_file_tab3.read()
                    
                st.success("Record Uang Kembali (UK) berhasil ditambahkan!")
                safe_rerun()

    # ---------------- TAB 4: LAPORAN EBOS ----------------
    with tab4:
        st.markdown("### 📑 Form Input Registrasi Laporan EBOS")
        with st.form("form_input_ebos_tab"):
            e_1, e_2, e_3 = st.columns(3)
            with e_1:
                e_no = st.text_input("No Tiket EBOS", f"EBOS-202607-00{len(st.session_state.df_ebos)+1}", key="e_no_tab")
                e_wsid = st.text_input("WSID Terminal", "ZDA3", key="e_wsid_tab")
                e_lokasi = st.text_input("Lokasi Terminal", "WINGS PROBOLINGGO", key="e_lokasi_tab")
            with e_2:
                e_tgl = st.date_input("Tanggal Laporan", key="e_tgl_tab")
                e_mesin = st.selectbox("Tipe Mesin", ["O-CRM", "ITM", "CRMHYO", "ACH"], key="e_mesin_tab")
                e_nom = st.number_input("Nominal Selisih EBOS (Rp)", value=500000, step=50000, key="e_nom_tab")
            with e_3:
                e_status = st.selectbox("Status Tiket", ["Open", "In Review", "Resolved", "Closed"], key="e_status_tab")
                e_catatan = st.text_area("Catatan Laporan EBOS", "Selisih ebos dalam verifikasi dokumen", key="e_catatan_tab")

            uploaded_file_tab4 = st.file_uploader("📸 Upload Foto Tiket/Struk EBOS (Opsional):", type=["png", "jpg", "jpeg"], key="upload_tab4")
            ...
            if ebos_submit:
                ...
                if uploaded_file_tab4 is not None:
                    st.session_state.image_store[e_wsid] = uploaded_file_tab4.read()
                    
                st.success(f"Tiket EBOS {e_no} berhasil dicatat!")
                safe_rerun()

    # ---------------- TAB 5: REKON EJ LOG ----------------
    with tab5:
        st.markdown("### 💻 Form Input Log Rekon Electronic Journal (EJ)")
        with st.form("form_input_ej_tab"):
            j_1, j_2, j_3 = st.columns(3)
            with j_1:
                j_wsid = st.text_input("WSID Terminal", "Z5UM", key="j_wsid_tab")
                j_lokasi = st.text_input("Nama Lokasi", "PURWOSARI CRM 2", key="j_lokasi_tab")
                j_tgl = st.date_input("Tanggal Log EJ", key="j_tgl_tab")
            with j_2:
                j_jam = st.text_input("Jam Transaksi", "20:57", key="j_jam_tab")
                j_kartu = st.text_input("No Kartu / Rekening", "5379-XXXX-1029", key="j_kartu_tab")
                j_nom = st.number_input("Nominal Transaksi EJ (Rp)", value=100000, step=50000, key="j_nom_tab")
            with j_3:
                j_status = st.selectbox("Status Matching EJ", ["Matched", "Discrepancy - Short", "Discrepancy - Over"], key="j_status_tab")
                j_catatan = st.text_area("Catatan Temuan EJ", "Tx Jam 20:57 dispenser d.100 x 1 lbr match", key="j_catatan_tab")

            uploaded_file_tab5 = st.file_uploader("📸 Upload Foto Struk/Pita EJ (Opsional):", type=["png", "jpg", "jpeg"], key="upload_tab5")

            ej_submit = st.form_submit_button("💾 Simpan Record Log EJ")
            if ej_submit:
                new_j = {
                ...
                if uploaded_file_tab5 is not None:
                    st.session_state.image_store[j_wsid] = uploaded_file_tab5.read()
                    
                st.success("Record Log EJ berhasil disimpan!")
                safe_rerun()

    # ---------------- TAB 6: SISLOK STOCK REVIEW ----------------
    with tab6:
        st.markdown("### 📦 Form Input Review Stock Sislok")
        with st.form("form_input_sislok_tab"):
            s_1, s_2, s_3 = st.columns(3)
            with s_1:
                s_wsid = st.text_input("WSID Terminal", "Z0X2", key="s_wsid_tab")
                s_lokasi = st.text_input("Lokasi Terminal", "IDM SUDIMORO MALANG", key="s_lokasi_tab")
                s_tgl = st.date_input("Tanggal Audit Stock", key="s_tgl_tab")
            with s_2:
                s_awal = st.number_input("Stok Sislok Awal (Ribu Rp)", value=600000, step=10000, key="s_awal_tab")
                s_masuk = st.number_input("Total Kas Masuk (Ribu Rp)", value=649850, step=10000, key="s_masuk_tab")
                s_keluar = st.number_input("Total Kas Keluar (Ribu Rp)", value=649800, step=10000, key="s_keluar_tab")
            with s_3:
                s_sislok_akhir = s_awal + s_masuk - s_keluar
                st.info(f"Stok Sislok Akhir Terkalkulasi: **Rp {s_sislok_akhir:,.0f} (Ribu Rp)**")
                s_fisik = st.number_input("Fisik Kas Actual (Ribu Rp)", value=600000, step=10000, key="s_fisik_tab")
                s_selisih = s_fisik - s_sislok_akhir
                s_status = "Balanced" if s_selisih == 0 else "Variance Detected"

            uploaded_file_tab6 = st.file_uploader("📸 Upload Foto Stock Sheet / Opname (Opsional):", type=["png", "jpg", "jpeg"], key="upload_tab6")
            ...
            if sislok_submit:
                ...
                if uploaded_file_tab6 is not None:
                    st.session_state.image_store[s_wsid] = uploaded_file_tab6.read()
    ...
    # ---------------- TAB 7: UANG KOLONG ----------------
    with tab7:
        st.markdown("### 🪙 Form Input Temuan Uang Kolong")
        with st.form("form_input_kolong_tab"):
            k_1, k_2, k_3 = st.columns(3)
            with k_1:
                k_wsid = st.text_input("WSID Terminal", "Z5UM", key="k_wsid_tab")
                k_lokasi = st.text_input("Nama Lokasi", "PURWOSARI CRM 2", key="k_lokasi_tab")
                k_tgl = st.date_input("Tanggal Temuan", key="k_tgl_tab")
            with k_2:
                k_denom = st.selectbox("Denominasi Uang", ["100,000", "50,000", "20,000"], key="k_denom_tab")
                k_lembar = st.number_input("Jumlah Lembar", value=1, min_value=1, key="k_lembar_tab")
            ...
            with k_3:
                k_posisi = st.selectbox("Lokasi Temuan Fisik", ["Kolong Dispenser", "Fascia Roll / Bawah", "Reject Box Chamber", "Transport Mechanism"], key="k_posisi_tab")
                k_ket = st.text_area("Keterangan Fisik", "Ditemukan terselip saat bongkar isi kas", key="k_ket_tab")

            uploaded_file_tab7 = st.file_uploader("📸 Upload Foto Fisik Uang Kolong (Opsional):", type=["png", "jpg", "jpeg"], key="upload_tab7")
            ...
            if kolong_submit:
                ...
                if uploaded_file_tab7 is not None:
                    st.session_state.image_store[k_wsid] = uploaded_file_tab7.read()
                    
                st.success("Temuan Uang Kolong berhasil dicatat!")
                safe_rerun()

    # ---------------- TAB 8: UPLOAD FOTO BUKTI TABEL ----------------
    with tab8:
        st.markdown("### 📸 Unggah & Lampirkan Foto Bukti Khusus WSID")
        col_up1, col_up2 = st.columns([1, 1])

        with col_up1:
            st.markdown("#### 📤 Pilih WSID & Unggah Gambar")
            target_wsid_input = st.selectbox("Pilih WSID Target:", df_rekon["WSID"].unique(), key="target_wsid_input_tab8")
            uploaded_img_direct = st.file_uploader("Unggah File Gambar Foto Bukti (PNG / JPG / JPEG):", type=["png", "jpg", "jpeg"], key="upload_direct_tab8")

            if uploaded_img_direct is not None:
                image_bytes = uploaded_img_direct.read()
                st.session_state.image_store[target_wsid_input] = image_bytes
                st.success(f"Foto bukti untuk WSID {target_wsid_input} berhasil disimpan!")

        with col_up2:
            st.markdown("#### 🖼️ Pratinjau Foto Bukti")
            if target_wsid_input in st.session_state.image_store:
                st.image(st.session_state.image_store[target_wsid_input], caption=f"Bukti Fisik WSID {target_wsid_input}", use_container_width=True)
            else:
                st.info(f"Belum ada foto bukti yang diunggah untuk WSID {target_wsid_input}.")
