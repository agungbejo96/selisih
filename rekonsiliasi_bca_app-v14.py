...
# ---------------------------------------------------------
st.set_page_config(
    page_title="Sistem Rekonsiliasi Kas BCA + Mode Input Bersih",
    page_icon="🏦",
    layout="wide",
...
# ---------------------------------------------------------
    ...
    with tab_c:
        ...
        with st.form("form_create_crud"):
            ...
            c_cash_pos = st.number_input("Cash Position (Ribu Rp)", value=500000)
            c_setor = st.number_input("Total Setor (Ribu Rp)", value=499500)
...
# ---------------------------------------------------------
    ...
    with tab1:
        ...
        with st.form("form_inp_rekon"):
            ...
            with c1:
                i_wsid = st.text_input("WSID Terminal", "Z77M")
                i_lok = st.selectbox("LOK (Pengelola)", ["BCA", "T"])
                i_lokasi = st.text_input("Nama Lokasi", "KCP MALANG KOTA")
            with c2:
            ...
            with c3:
                i_cash_pos = st.number_input("Cash Position Sistem (Ribu Rp)", value=500000)
                i_setor = st.number_input("Total Disetor (Ribu Rp)", value=499500)
                i_sel_awal = i_setor - i_cash_pos
                st.info(f"Estimasi Selisih Awal: **{i_sel_awal:,.0f} (Ribu Rp)**")
            with c4:
                i_sel_akhir = st.number_input("Selisih Akhir Audit (Ribu Rp)", value=-100)
                i_uk_bdc = st.text_input("Catatan UK BDC", "Rp 500,000 (UK Siclus)")
                i_test_cash = st.text_input("Catatan Tes Cash", "Rp 100,000 (NON-BDC)")

            c5, c6, c7, c8 = st.columns(4)
            with c5:
                i_staff1 = st.text_input("Petugas 1", "BAYU OKTA PURWANTO")
            with c6:
                i_staff2 = st.text_input("Petugas 2", "AKHMAD RIZA MAULANA")
            with c7:
                i_staff3 = st.text_input("Petugas 3", "YOYOK BUDI RAKHMAN")
            with c8:
                i_driver = st.text_input("Driver Pengawal", "ASSYAVI ALIYULLOH M")

            i_activity = st.text_input("Aktivitas Operasional", f"{datetime.now().strftime('%d/%m/%y %H:%M')} - BONGKAR ISI")
            i_catatan = st.text_input("Catatan Lapangan", "Proses normal, UK d.100 x 1 lbr")
            i_tanggapan = st.text_area("Tanggapan BCA", f"Selisih Awal : {i_sel_awal}\nUK : {abs(i_sel_awal - i_sel_akhir)}\nSelisih Akhir : {i_sel_akhir}")
    ...
    with tab2:
        ...
        with st.form("form_inp_cencon"):
            ...
            with c_c1:
                cn_wsid = st.text_input("WSID Terminal", "Z5UM")
                cn_lokasi = st.text_input("Nama Lokasi", "CRM PURWOSARI 2")
            with c_c2:
                cn_tgl = st.date_input("Tanggal Transaksi")
                cn_jam = st.text_input("Jam (HH:MM)", "20:22")
            with c_c3:
                cn_bdc = st.number_input("Nominal BDC (Rp)", value=0, step=10000)
                cn_non_bdc = st.number_input("Nominal NON-BDC (Rp)", value=100000, step=10000)
            with c_c4:
                cn_status = st.selectbox("Status Cencon", ["Close", "Open", "Pending"])
                cn_ket = st.text_input("Keterangan Tes Cash", "Tes cash sesuai 100,000")
    ...
    with tab8:
        ...
        target_wsid_tab = st.text_input("WSID Target Foto:", "Z5UM")
