...
# ---------------------------------------------------------
st.set_page_config(
    page_title="Sistem Rekonsiliasi Kas BCA + Periode Bulan dari TGL_REM",
    page_icon="🏦",
    layout="wide",
...
# Helper to parse any date string/obj to Indonesian Month Format (e.g. "Juli 2026")
def parse_indonesian_bulan(target_date):
    if pd.isna(target_date) or str(target_date).strip() in ["", "-", "nan", "None"]:
        return None
    
    val_str = str(target_date).strip()
    month_names = {
    ...
    indo_months = {
        ...
        "agu": 8, "agustus": 8, "agust": 8, "sep": 9, "september": 9,
    ...
    for m_name, m_num in indo_months.items():
        if m_name in lower_val:
            years = re.findall(r'\b(20\d{2}|\d{2})\b', lower_val)
            if years:
                yr = int(years[-1])
                if yr < 100:
                    yr += 2000
                return f"{month_names[m_num]} {yr}"
...
# Auto fill/extract BULAN from TGL_REM (Tanggal Removal) with Indonesian Month Format
def auto_fill_bulan(df, date_col="TGL_REM"):
    ...
    for idx, row in df.iterrows():
        ...
        if val in ["-", "", "nan", "None"] or pd.isna(row.get("BULAN")):
            ...
            res = parse_indonesian_bulan(target_date)
            if res:
                df.at[idx, "BULAN"] = res
            else:
...
# ---------------------------------------------------------
    ...
    # Dynamic Column & Header Adjustment: Displays all columns/headers from uploaded file
    predefined_cols = [
    ...
    # Keep standard columns in order if present, plus any newly uploaded custom columns/headers
