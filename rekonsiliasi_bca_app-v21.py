...
# ---------------------------------------------------------
st.set_page_config(
    page_title="Sistem Rekonsiliasi Kas BCA + Table Edit, Update & Delete Actions",
    page_icon="🏦",
    layout="wide",
...
# Auto fill/extract BULAN from TGL_REM (Tanggal Removal)
def auto_fill_bulan(df, date_col="TGL_REM"):
    ...
    # Identify column for Tanggal Removal (TGL_REM)
    rem_col = None
    for c in df.columns:
    ...
    if not disp_df.empty:
        ...
        with tab_act_d:
            ...
            if st.button(f"🗑️ [DELETE] Konfirmasi Hapus Data WSID {sel_d_wsid}", type="primary", key="btn_confirm_del"):
                ...
                st.session_state.upload_success_msg = f"🗑️ Data WSID '{sel_d_wsid}' berhasil dihapus dari Tabel Utama!"
                safe_rerun()

        with tab_act_a:
            with st.form("form_main_add_new"):
                st.info("➕ Tambahkan Data Rekonsiliasi Baru Langsung ke Tabel Utama")
                a_c1, a_c2, a_c3, a_c4 = st.columns(4)
                with a_c1:
                    ma_wsid = st.text_input("WSID Terminal Baru", "")
                    ma_lok = st.selectbox("LOK", ["BCA", "T"], index=0)
                    ma_lokasi = st.text_input("Lokasi Terminal Baru", "")
                with a_c2:
                    ma_mesin = st.selectbox("Tipe Mesin Terminal", ["O-CRM", "ITM", "CRMHYO", "ACH"])
                    ma_tgl_ins = st.date_input("Tanggal Insert")
                    ma_tgl_rem = st.date_input("Tanggal Removal")
                with a_c3:
                    ma_cash_pos = st.number_input("Cash Position (Ribu Rp)", value=0)
                    ma_setor = st.number_input("Total Setor (Ribu Rp)", value=0)
                    ma_sel_akhir = st.number_input("Selisih Akhir (Ribu Rp)", value=0)
                with a_c4:
                    ma_uk_bdc = st.text_input("Catatan UK BDC", "")
                    ma_test_cash = st.text_input("Catatan Tes Cash", "")
                    ma_staff1 = st.text_input("Petugas 1", "")

                ma_activity = st.text_input("Aktivitas Operasional", "")
                ma_catatan = st.text_input("Catatan Lapangan", "")
                
                sub_main_add = st.form_submit_button("💾 [CREATE] Simpan Data Baru ke Tabel Utama", type="primary")
                if sub_main_add and ma_wsid:
                    new_row_entry = {
                        "WSID": ma_wsid, "LOK": ma_lok, "LOKASI": ma_lokasi, "Mesin": ma_mesin,
                        "TGL_INS": pd.to_datetime(ma_tgl_ins), "TGL_REM": pd.to_datetime(ma_tgl_rem),
                        "CASH_POS": ma_cash_pos, "SETOR": ma_setor,
                        "SEL_AWAL": ma_setor - ma_cash_pos, "SEL_AKHIR": ma_sel_akhir,
                        "UK_BDC": ma_uk_bdc, "TEST_CASH": ma_test_cash, "CLOSE_OPEN_CENCON": "Close",
                        "REKON_SISLOK": "Balanced", "REVIEW_STOCK": f"Sislok: {ma_cash_pos:,.0f} | Fisik: {ma_setor:,.0f}",
                        "STAFF_1": ma_staff1, "ACTIVITY": ma_activity, "CATATAN_PROSES": ma_catatan
                    }
                    st.session_state.df_rekon = pd.concat([st.session_state.df_rekon, pd.DataFrame([new_row_entry])], ignore_index=True)
                    st.session_state.df_rekon = auto_fill_bulan(st.session_state.df_rekon, "TGL_REM")
                    st.session_state.upload_success_msg = f"🎉 Data baru WSID '{ma_wsid}' berhasil ditambahkan ke Tabel Utama!"
                    safe_rerun()
    else:
        st.info("ℹ️ Tabel saat ini masih kosong. Silakan gunakan form Tambah Data di atas atau unggah file Excel.")
