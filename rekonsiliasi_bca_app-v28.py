...
# ---------------------------------------------------------
st.set_page_config(
    page_title="Abacus Malang - Cash Reconciliation & Executive System",
    page_icon="🏦",
    layout="wide",
...
<div style="text-align: center; padding: 14px 10px 18px 10px; background: #FFFFFF; border-radius: 14px; border: 1px solid #E2E8F0; margin-bottom: 22px; box-shadow: 0 6px 16px -2px rgba(0, 43, 73, 0.08);">
    {get_abacus_logo_html(width=205, height=58)}
...
""", unsafe_allow_html=True)
st.sidebar.title("Navigasi Operasional Abacus Malang")
...
# ---------------------------------------------------------
    ...
    predefined_cols = [
        ...
        "DAFTAR_STAFF_LENGKAP", "STAFF_1", "STAFF_2", "STAFF_3", "STAFF_LAINNYA", "DRIVER", "ACTIVITY", "CATATAN_PROSES", "TANGGAPAN_ABACUS"
    ]
...
# ---------------------------------------------------------
    ...
    with col_ex1:
        ...
        st.download_button(
            ...
            file_name=f"Rekonsiliasi_Lengkap_Abacus_Malang_{datetime.now().strftime('%Y%m%d')}.xlsx",
