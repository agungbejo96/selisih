...
# ---------------------------------------------------------
st.set_page_config(
    page_title="Sistem Rekonsiliasi Kas BCA + Fitur Reset / Kosongkan Data",
    page_icon="🏦",
    layout="wide",
...
def reset_all_data_empty():
    st.session_state.df_rekon = pd.DataFrame(columns=[
        "WSID", "LOK", "LOKASI", "Mesin", "BULAN", "TGL_INS", "TGL_REM", 
        "CASH_POS", "SETOR", "SEL_AWAL", "SEL_AKHIR", "UK_BDC", "TEST_CASH", 
        "CLOSE_OPEN_CENCON", "REKON_SISLOK", "REVIEW_STOCK", "STAFF_1", 
        "STAFF_2", "STAFF_3", "DRIVER", "ACTIVITY", "CATATAN_PROSES", "TANGGAPAN_BCA"
    ])
    st.session_state.df_cencon = pd.DataFrame(columns=[
        "WSID", "LOKASI", "TANGGAL", "JAM", "BDC", "NON_BDC", "STATUS", "KETERANGAN"
    ])
    st.session_state.df_uk = pd.DataFrame(columns=[
        "WSID", "LOKASI", "TANGGAL", "JAM", "TIPE_UK", "BDC", "NON_BDC", "TOTAL_UK", "KETERANGAN"
    ])
    st.session_state.df_ebos = pd.DataFrame(columns=[
        "NO_LAPORAN", "WSID", "LOKASI", "TANGGAL", "TIPE_MESIN", "NOMINAL_SELISIH", "STATUS_EBOS", "CATATAN_EBOS"
    ])
    st.session_state.df_ej = pd.DataFrame(columns=[
        "WSID", "LOKASI", "TANGGAL_EJ", "JAM_TX", "NO_KARTU_REK", "NOMINAL_EJ", "STATUS_EJ", "CATATAN_EJ"
    ])
    st.session_state.df_sislok = pd.DataFrame(columns=[
        "WSID", "LOKASI", "TANGGAL_AUDIT", "STOK_SISLOK_AWAL", "KAS_MASUK", "KAS_KELUAR", "STOK_SISLOK_AKHIR", "FISIK_KAS_ACTUAL", "SELISIH_STOK", "STATUS_REVIEW"
    ])
    st.session_state.df_kolong = pd.DataFrame(columns=[
        "WSID", "LOKASI", "TANGGAL_TEMUAN", "DENOMINASI", "LEMBAR", "TOTAL_RUPIAH", "LOKASI_TEMUAN", "KETERANGAN"
    ])
    st.session_state.image_store = {}
...
if menu == "📊 Dashboard Executive Overview":
    ...
    with col2:
        tot_pos = safe_float(pd.to_numeric(filtered_df["CASH_POS"], errors="coerce").sum()) * 1000 if "CASH_POS" in filtered_df.columns else 0.0
        st.metric("Total Cash Position", f"Rp {safe_float(tot_pos):,.0f}")
    with col3:
        tot_setor = safe_float(pd.to_numeric(filtered_df["SETOR"], errors="coerce").sum()) * 1000 if "SETOR" in filtered_df.columns else 0.0
        st.metric("Total Disetor", f"Rp {safe_float(tot_setor):,.0f}")
    with col4:
        tot_sel_awal = safe_float(pd.to_numeric(filtered_df["SEL_AWAL"], errors="coerce").sum()) * 1000 if "SEL_AWAL" in filtered_df.columns else 0.0
        st.metric("Total Selisih Awal", f"Rp {safe_float(tot_sel_awal):,.0f}", delta=f"{safe_float(tot_sel_awal):,.0f}", delta_color="inverse")
    with col5:
        tot_sel_akhir = safe_float(pd.to_numeric(filtered_df["SEL_AKHIR"], errors="coerce").sum()) * 1000 if "SEL_AKHIR" in filtered_df.columns else 0.0
        st.metric("Total Selisih Akhir", f"Rp {safe_float(tot_sel_akhir):,.0f}", delta=f"{safe_float(tot_sel_akhir):,.0f}", delta_color="inverse")
    ...
    if "BULAN" in filtered_df.columns and not filtered_df.empty:
        b_chart_df = filtered_df.copy()
        for col_n in ["SEL_AWAL", "SEL_AKHIR", "CASH_POS", "SETOR"]:
            b_chart_df[col_n] = pd.to_numeric(b_chart_df[col_n], errors="coerce").fillna(0)
        df_monthly_chart = b_chart_df.groupby("BULAN")[["SEL_AWAL", "SEL_AKHIR", "CASH_POS", "SETOR"]].sum().reset_index()
...
# ---------------------------------------------------------
    ...
    if "BULAN" in filtered_df.columns and not filtered_df.empty:
        ...
        for idx, b_name in enumerate(bulan_list):
            ...
            b_sel_awal = safe_float(pd.to_numeric(b_df["SEL_AWAL"], errors="coerce").sum()) * 1000
            b_sel_akhir = safe_float(pd.to_numeric(b_df["SEL_AKHIR"], errors="coerce").sum()) * 1000
            b_count = len(b_df)
    ...
    if not disp_df.empty:
        ...
        with tab_dt1:
            ...
            with c3:
                ...
                sel_awal_v = safe_float(wsid_row.get('SEL_AWAL', 0)) * 1000
                sel_akhir_v = safe_float(wsid_row.get('SEL_AKHIR', 0)) * 1000
                st.write(f"**Selisih Awal:** Rp {sel_awal_v:,.0f}")
                st.write(f"**Selisih Akhir:** Rp {sel_akhir_v:,.0f}")
